...
from the top with brightness and volume sliders, a search that drops from the
home screen, notification banners, a lock screen with a passcode pad -- with
every app on the same frosted rounded-square tile, and Omarchy's see-through
surfaces over a blur instead of Apple's liquid glass.
...
plugins/         shell plugins, installed to ~/.config/omarchy/plugins
  ...
  phone-lock         lock screen: Omarchy's lock service with its view
                     swapped for LockView.qml (the time, then a passcode
                     pad), built on the phone by omarchy-phone-lock-build
...
| `omarchy-phone-lock-build` | build the lock screen from the installed Omarchy's lock service (`--clean` removes the build) |
